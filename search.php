...
$news_rs_total = $wpdb->query(' SELECT * FROM `'.$fiu.'` '.(!empty($c_where) ? ' WHERE ' . $c_where : ''));
...
$news_rs = $wpdb->get_results(' SELECT * FROM `'.$fiu.'` '.(!empty($c_where) ? ' WHERE ' . $c_where : '') . ' '.($c_order ? ' ORDER BY ' . $c_order : '').' LIMIT '.$offset.',10 ');
if(!empty($news_rs)){
